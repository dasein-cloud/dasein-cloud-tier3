...
public class APIHandler {
    ...
    public APIHandler(@Nonnull Tier3 provider) { this.provider = provider; }
    ...
    public void delete(@Nonnull String resource, @Nonnull String id, @Nullable NameValuePair ... parameters) throws InternalException, CloudException {
        if( logger.isTraceEnabled() ) {
            logger.trace("ENTER - " + APIHandler.class.getName() + ".delete(" + resource + "," + id + "," + Arrays.toString(parameters) + ")");
        }
        try {
            ...
            try {
                ...
                try {
                    ...
                    if( status.getStatusCode() != NO_CONTENT ) {
                        ...
                        if( entity == null ) {
                            throw new Tier3Exception(CloudErrorType.GENERAL, status.getStatusCode(), status.getReasonPhrase(), status.getReasonPhrase());
                        }
                        String body;
                        ...
                        wire.debug("");
                        throw new Tier3Exception(CloudErrorType.GENERAL, status.getStatusCode(), status.getReasonPhrase(), body);
                    }
                }
                finally {
    ...
    private void get(@Nonnull APIResponse apiResponse, @Nullable String paginationId, final int page, final @Nonnull String resource, final @Nullable String id, final @Nullable NameValuePair ... parameters) throws InternalException, CloudException {
        if( logger.isTraceEnabled() ) {
            logger.trace("ENTER - " + APIHandler.class.getName() + ".get(" + paginationId + "," + page + "," + resource + "," + id + "," + Arrays.toString(parameters) + ")");
        }
        try {
            ...
            try {
                ...
                try {
                    ...
                    if( status.getStatusCode() != OK ) {
                        ...
                        wire.debug("");
                        apiResponse.receive(new Tier3Exception(CloudErrorType.GENERAL, status.getStatusCode(), status.getReasonPhrase(), body));
                    }
                    else {
                        HttpEntity entity = response.getEntity();
            ...
            try {
                ...
                try {
                    ...
                    if( status.getStatusCode() != ACCEPTED && status.getStatusCode() != CREATED ) {
                        ...
                        wire.debug("");
                        throw new Tier3Exception(CloudErrorType.GENERAL, status.getStatusCode(), status.getReasonPhrase(), json);
                    }
                    else {
                        HttpEntity entity = response.getEntity();
